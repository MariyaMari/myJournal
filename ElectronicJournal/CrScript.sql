...
create table link1(id_spec integer references specialty(id_spec) on update cascade on delete cascade, n_rab varchar(100) unique, rabn integer, id_trab integer references typerabot(id_trab) on update cascade on delete cascade, semes integer, id_dis integer references disciplina(id_dis) on update cascade on delete cascade, nedel integer, rang integer, id_lin1 integer unique, primary key(id_spec, rabn, id_trab, semes, id_dis));
